...
В `activity_main.xml` добавлены `TextView` и кнопка «Запустить таймер». При нажатии на кнопку запускается `Timer`, который через 5 секунд изменяет текст в `TextView`. Обновление UI выполнено через `runOnUiThread()`, так как `TimerTask` работает в фоновом потоке.

![Таймер через 5 секунд](media/3.png)

**Рисунок 3** — Изменение текста после задержки
...
![Секундомер в работе](media/4.png)

**Рисунок 4** — Интерфейс секундомера

---

## Задания для самостоятельного выполнения

Выбран **вариант 1: сумма простых чисел**. Реализовано приложение, которое каждую секунду вычисляет сумму всех простых чисел в диапазоне от 0 до 100. Каждую секунду текущая сумма выводится в Logcat (тег `Lab6`) и отображается в `TextView`. По достижении конца диапазона таймер останавливается, выводится финальная сумма.

![Результат работы таймера](media/5.png)

**Рисунок 5** — Отображение текущей суммы

![Логирование промежуточных значений](media/6.png)

**Рисунок 6** — Логи в Logcat (тег Lab6)
